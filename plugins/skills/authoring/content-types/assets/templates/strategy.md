...
<!-- 執筆指示: 記載例を先に読む: [`../examples/strategy.example.md`](../examples/strategy.example.md)
     ...
     導入には資源配分の判断に使う範囲を残し、North Star自体の再定義や詳細な作業計画と区別する。
     「やらないこと」は資源を配らない事業上の選択であり、文書の対象外を書く欄ではない。
     項目を埋めることより、前の判断から次の判断へ因果が読めることを優先する。
     執筆指示と未置換のプレースホルダーを成果物から削除する。 -->
...
<!-- 行動の数は固定しない。**2つで基本方針を実現できるなら2つでよい。**
     3つ目の欄を埋めるために行動を作らない。逆に4つ要るなら足す。 -->
