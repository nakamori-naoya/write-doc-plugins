...
<!-- 執筆指示: 業務知識・コアドメインの正本に現れる業務概念を、値オブジェクト・エンティティ・集約・ドメインイベントへ割り当て、
     ...
     読み手は、業務を形にするエンジニア（何を要素にし、どこに境界があり、どの状態で何ができるかを確認する）、レビューする人（割り当ての理由・捨てた案・未決から正本のどこから来たかを辿る）、
     正本を反証・更新する人（「正本へ提案する概念」を入力にする）、データモデルや実装の担当者（この資料を入力に永続化や型設計を決める）の4者で、主な読み手は最初の1者である。
     不変条件は3つ。
...
<!-- 機械検査が読む記法（domain-modelingの検査scriptはこの記法だけを構文解析する。ここに1度だけ書く）:
     ...
     - `## 要素一覧` は `| 要素 | 種別 | 正本の語 | 目的（一文） |` の表。空欄を置かず、同じ要素を2度書かない。種別は 値オブジェクト／エンティティ／集約ルート／ドメインイベント のどれか（ドメインサービスは例外扱いとして報告に出る）。要素名と正本の語は、正本（domain-rule）から機械抽出した索引にある語をそのまま使う。
     ...
     - `## BDDとの対応` は `| BDD | 要素 | 操作 | 成立を決める不変条件・事前条件 |` の表と、`- 対応しないBDD:` `- 対応のない要素・操作:` の2行を持つ。正本の全BDD番号が表か前者の行に、要素一覧の全要素が表か後者の行に現れる。正本に無いBDD番号を書かない。
     - `## 正本へ提案する概念` は `| 概念 | なぜ要るか | 導いた業務ルール・BDD | 正本のどの節へ足すか |` の表（空欄なし、同じ概念を2度書かない、引くBDD番号は正本にあるもの、足す先は正本の節名）か、「なし」だけ。提案した語は要素一覧の要素名と正本の語に現れない。
     - `## 未決` は空にしない（0件なら「なし」）。 -->
