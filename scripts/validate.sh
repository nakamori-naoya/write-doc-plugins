...
expect "package version 9.4.2" jq -e '.version=="9.4.2"' "$CODEX"
...
for market in .claude-plugin/marketplace.json .agents/plugins/marketplace.json; do
  if jq -e '.name=="write-doc" and (.plugins|length)==1 and .plugins[0].name=="write-doc" and .plugins[0].version=="9.4.2"
