...
内容の正しさ、業務上の境界、BDDの妥当性は入力として与えられた事実から判断する。この文書型は業務知識を発見・分類・補完せず、同梱の骨格と記載例だけを示す。
...
要素の割り当て、集約の境界、状態ごとの操作の可否、BDDとの対応の妥当性は入力として与えられた事実から判断する。この文書型はモデルを設計せず、同梱の骨格と記載例だけを示す。永続化・層構成・画面・APIは、この型では検討先を示すだけで扱わない。
