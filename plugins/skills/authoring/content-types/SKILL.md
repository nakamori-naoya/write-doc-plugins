...
**選んだ型・catalogのslug・理由を1行で宣言する。** 読み手か目的が不明なら聞く。playbookから呼ばれた場合は直接聞かず、決まらない問いを推奨回答付きで`open_questions`へ残す（先頭に`count: <件数>`、無ければ`count: 0`）。後続のsettle工程がgrillで合意を取る。両方が判明してもカタログ上で一意に定まらない場合だけ、`${.default_type}`をslugとして選ぶ。
