...
<!-- 執筆指示: この資料では、業務知識のコマンドが作成、更新、削除するデータを、テーブルの形と具体的な行で議論し、何を時間を越えて残すか、起きたことをどこに積むかを決める。読み取りはクエリデータモデルが扱う。
     ...
     対象ごとに型を選ぶ。業務イベントが複数あり、その起こる順番を記録する要求などがあれば、イミュータブルデータモデル（リソースと基底イベント）で書く。「設定を更新する」のような言い回ししかしない対象は、リソース中心（今の状態だけを持つ表）で書く。履歴を残したい要求があれば、履歴の表を考える。選んだ型と、その根拠（要件と業務知識のどの文か）を、分類の表の下に対象ごとに一文で書く。
     イミュータブルデータモデルのリソースには、生まれたときに業務が与えた値と、status と current_version を置く。状態が一つしか無くても外さない。status の値は、業務知識のユビキタス言語で種類が `状態` の語の英名を snake_case にしたものを使う。業務知識が状態を持たない対象では、status はその対象を生んだ業務イベントの基底イベントの種類と同じ値、current_version は 1 にし、状態名を作らない。
     イミュータブルデータモデルでは、業務の行いによる終わりは、名前が「消す」でも新しい事実を足して表す。リソース中心の対象は、業務の行いで行を消してよい。
     後から導けず、失うと業務が続かない技術のデータ（決済事業者が返した取引の識別子など）は、系列を `リソース系`、性質を `技術` にしたテーブルで書き、根拠の欄は要件の品質要求を指してよい。status と current_version はイミュータブルデータモデルの業務のリソースにだけ置く。基底イベントには適用した後の版 version を置き、イベントの表の時刻は occurred_at の一本にする。
...
<!-- 検査が読む目印: ほかの package の検査がこの型の資料から読むのは、ここに書いた目印だけである。見出しの文言は読まないので、見出しには節の結論を入れてよい。
     ...
     持ち主の資料がまだ無いテーブルは、参照の表の読む列の欄に `未定` と書き、図にも BDD の行にも載せない。テーブル名も分からないときは、テーブル名の欄にも `未定` と書く。持ち主の資料ができたら、同じ入口で深めて外す。
     持ち主が作業の範囲の外にあり、資料が置かれないときは、参照の表の持ち主の資料の欄に `範囲の外: [<持ち主の資料の名前>](<パスかURL>)` と書く。その行はほかの資料と照合されない。
...
<対象ごとに一文で、選んだ型と、その根拠になった要件と業務知識の文。リソースの単位をそう切った理由>
