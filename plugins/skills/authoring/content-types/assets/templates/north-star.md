...
<!-- 執筆指示: 記載例を先に読む: [`../examples/north-star.example.md`](../examples/north-star.example.md)
     ...
     基本方針、資源配分、期限つき行動はProduct Strategyへ委ねる。長期判断の基準として
     この文書を使う範囲を導入に残す。「プロダクトの役割」「やらないこと」は事業上の選択であり、
     文書が扱わない内容の説明で埋めない。節内はラベル付き箇条書きにせず、
     意味のある小見出しと本文で構成する。**節は経路表が決める。**到達点に不要な節は置かない。
     置いた節が埋められない場合だけ、未確認である理由と確認方法を書く。
     執筆指示と未置換のプレースホルダーは成果物から削除する。 -->
...
<!-- この図は、対象・損なわれた価値・プロダクトの役割・望む状態がひと続きにつながることを短時間で確かめるためのもの。
     ...
     本文に合わせて `north-star-value-flow.svg` の文言を置き換える。 -->

![対象の損なわれた価値から望む状態までをプロダクトの役割でつなぐ流れ](north-star-value-flow.svg)
...
<!-- この図は、担う範囲・他者が担う範囲・担う範囲の内側で採らない方針・境界で使う判断原則の4つを一枚で確かめるためのもの。
     ...
     本文に合わせて `north-star-boundary.svg` の文言を置き換える。 -->

![プロダクトが担う範囲と担わない範囲、担う範囲の内側で採らない方針、および境界で働く判断原則](north-star-boundary.svg)
