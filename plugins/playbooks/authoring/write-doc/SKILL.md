...
**曖昧さが残るなら、書く前に潰す。** `settle`工程は`when: open_questions.count > 0`の条件付きで、`grill`（grill@grill）を呼んで1問ずつ推奨付きで合意を取り、`decisions`を残す。問いが無ければ`state.py skip --step settle`で飛ばす。依頼が読み手・目的・型を明示している事項や、調べれば分かることをgrillで問い直さない。判断の詳細は[読者への引き継ぎ契約](references/reader-contract.md)。draft工程へ型・骨格と一緒に渡す。[読者への引き継ぎ契約](references/reader-contract.md)をここで読む。
