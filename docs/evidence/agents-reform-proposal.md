...
- このrepositoryの公開契約は`plugins/playbooks/authoring/write-doc/CONTRACT.md`が契約定義である。契約に無い振る舞いを消費側へ約束しない。契約変更時は両manifestの`contractVersion`と`implements`を一致させる。
...
- install cacheと配布済みcacheを編集しない。このsourceだけを変更対象とする。
