...
ほかの package からも同じ入力で呼び、`status` と `path`（失敗なら `reason`）を受け取る。ほかの package が読んでよいのは、書くときの規範と型ごとのテンプレートで、ほかの package の検査が資料から読んでよいのは、テンプレートの冒頭にある「検査が読む目印」だけである。
