...
<!-- 執筆指示: 作りを全部やめても残る業務の決まりを、業務の言葉と自然な文脈で残す。
     不変条件は3つ。
     1. コアドメインの業務知識に限る。判定は「作りを全部やめても残る決まりか」。残らないなら書かない。
        画面の作り・操作手順・保存の形・仕組み同士のやりとり・他業務の詳細手順は書かないが、なぜその業務ルールが存在し、破られると何が起きるのかの因果とメカニズム（決まりの根拠）は本文で丁寧に語る。
     2. 決まりを先、具体例（BDD）を資料の最後に。具体例より上だけを読んで業務の判断ができること。
        表の連打だけで終わらせず、業務の振る舞いや概念を読者が直感的にイメージできるよう平易な解説と具体例を添える。
     3. 業務の言葉だけを使う。業務を知る人が使わない語、まとまりの代表・境界・属性の一覧といった作り手の語彙は使わない。
     状態遷移の矢印ラベルには必ず引き金（誰の操作か、何が届いたか）を書き、条件は［...］で添える。
     時間の経過だけで移るなら「期限が来る」を引き金にし、その移り変わりを起こす行いを「コマンドとクエリ」にも載せる。状態を持つものが複数あるなら図を分ける。
     判定:
...
<!-- 書く: 誰が何をなぜ行う業務か。業務の目的と中核の価値を本文で語る。書かない: 実現手段、他が担う責務の詳細。 -->
<業務上の役割>
...
<!-- 書く: 業務上の行いの正式な定義。種別はコマンド（業務を変える行い）かクエリ（業務を変えずに知る行い）。
     すべての行いを載せ、業務イベントを起こさないクエリは「なし」と書く。
     期限の到来や別の業務イベントで状態が移るときも、それを起こす行い（例: 延滞にする）をコマンドとして載せる。
     ライフサイクルの対（開始と解除、登録と取消など）は片方だけを載せず、対象外なら対象外と書く。
     この表の第1列は「誰が行えるか」の第1列と一致させる。 -->
...
<!-- 誰が行えるかは、成立条件と同じ重さのコアの決まりである。
     書く: すべてのコマンドとクエリに行を置く（「コマンドとクエリ」表の第1列と一致させる）。
     主体の一致（本人のみ、など）は行える役割の欄に書く。期限の到来や別の出来事で起きる行いは、業務上の担い手を書く。
     どの仕組みが確かめるか（認証、認可、画面の制御）は業務の決まりではないので書かない。
     判定は「破られたときに困るのは業務か運用か」。運用なら書かない: トークン形式・有効期限、IP/利用者単位のレート制限、管理者操作。
     書かない: 権限の実現方法の詳細と設定値。 -->
...
<!-- 書く: 上で確定した決まりを、独立したGiven / When / Thenシナリオとして具体化する。
     ...
     - 「誰が行えるか」の行と対応するBDD（本人以外が行ったときの拒否）を、成立条件のBDDと同じ重さで置く。
...
<!-- 見出しは `### [BDD-<3桁以上の連番>] <業務結果>` で固定する。`Scenario` などの語を見出しへ入れない。
