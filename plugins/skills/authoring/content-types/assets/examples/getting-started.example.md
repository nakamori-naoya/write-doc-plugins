...
このガイドは「初回に1件作れるか」だけを確認する。確定、取消、順番待ち、権限設定は扱わない。
...
HTTP 201と予約ID`R-20260901-0101`、状態`tentative`、受付から15分後の`expires_at`が返れば成功である。次は2026-09-01 09:00に受け付けた場合の説明用レスポンスである。
...
同じcommandを再実行しても同じ予約IDが返り、予約が増えないことも確認する。`expires_at`は予約を確定するまでの期限であり、9月18日の利用時間を短くする値ではない。
