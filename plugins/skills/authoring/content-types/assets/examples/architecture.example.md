...
> 型: システム構成 ／ 読み手: RoomFlowを変更・運用するバックエンドエンジニア ／ 観測時点: 2026-08-28

**RoomFlowは、Cloud Run上のAPI 1つとワーカー1つが、Cloud SQL（PostgreSQL）1台を共有して動く。** 利用者からの入口はロードバランサ1つで、仮押さえ期限の到来だけがCloud Tasks経由の非同期処理である。組織のIDプロバイダより外へ出る通信は無い。
...
|---|---|---|
| 同時受付 | 現在の最大は80件/分（2026-08の1か月、1分ごとの受付件数の最大値。Cloud Loggingで集計）。Cloud Runの最大インスタンス数は10で、推定150〜200件/分まで受けられる（1インスタンスあたり15〜20件/分の実測から） | roomflow-api、roomflow-db |
