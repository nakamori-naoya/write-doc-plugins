...
<!-- 執筆指示: 業務知識・コアドメインの正式な定義に現れる業務概念を、値オブジェクト・エンティティ・集約・ドメインイベントへ割り当て、
     各要素の目的・何でないか・不変条件・操作の契約（事前条件・事後条件・拒む理由）を、言語非依存で書く。実装コードではない。
     読み手は、業務を形にするエンジニア（何を要素にし、どこに境界があり、どの状態で何ができるかを確認する）、レビューする人（割り当ての理由・捨てた案・未決から業務知識のどこから来たかを辿る）、
     業務知識を反証・更新する人（「業務知識へ提案する概念」を入力にする）、データモデルや実装の担当者（この資料を入力に永続化や型設計を決める）の4者で、主な読み手は最初の1者である。
     不変条件は3つ。
     1. 業務知識に無い語を要素にしない。要素の名前は業務知識で定義されたユビキタス言語の語だけを使う。必要に思えたら「業務知識へ提案する概念」へ、名前・なぜ要るか・導いた業務ルール・BDD・足す先の節を書き、業務知識の反証へ渡す。
     ...
     業務知識との対応: 業務用語→要素一覧の候補（値で区別→値オブジェクト、同一性で区別→エンティティ）、業務イベント→ドメインイベント（集約の操作が発するもの全部）、
...
<!-- 機械検査が読む記法（domain-modelingの検査scriptはこの記法だけを構文解析する。ここに1度だけ書く）:
     - H2見出しはこのtemplateの12節の名前と順序を固定する。追加・改名・並べ替えをせず、どの節も空にしない。テーブル定義・論理データモデル図・API・画面・リポジトリ・層構成という見出しを置かない。
     - `## 要素一覧` は `| 要素 | 種別 | 業務知識の語 | 目的（一文） |` の表。空欄を置かず、同じ要素を2度書かない。種別は 値オブジェクト／エンティティ／集約ルート／ドメインイベント のどれか（ドメインサービスは例外扱いとして報告に出る）。要素名と業務知識の語は、業務知識（domain-rule）から機械抽出した索引にある語をそのまま使う。
     - `## モデル図` は `## 集約の境界` の表にある集約ごとに `### 集約: <集約名>` を置き、直下に `- 責務:` と `- 境界:` の行、続けてMermaid classDiagramを1つ置く。`class <ASCIIのid>["<要素名>"]` のラベルは要素一覧の要素名だけを使い、集約名のクラスを必ず含める。classの中身は `<<種別>>` と `+<コマンド名>(<引数>)` だけで、フィールドやゲッターの行を書かない。要素一覧の全要素（ドメインイベントを含む）がどこかの集約の図に現れる。集約が2つ以上のときだけ `### 集約どうしの関係` を置いて全集約をclassにし、集約が1つなら置かない。
     - `## 各要素の詳細` はドメインイベント以外の全要素に `### <要素名>` を置き、その下に `#### 目的` `#### 何でないか` `#### 持つもの` `#### 不変条件` `#### 生成の条件` `#### 協働相手` の6項目すべてを空にせず書き（該当が無ければ「なし」）、これ以外の `####` 項目を置かない。`#### 操作: <操作名>` を1つ以上置き、各操作に `- 事前条件:` `- 事後条件:` `- 拒む理由:` `- 発するイベント:` の4行を空にせず書く。要素一覧に無い要素の詳細を置かない。
     - `## 集約の境界` は `| 集約 | ルート | 内側の要素 | 守る不変条件 | 外への参照 |` の表と `### 集約どうしの協働` を持つ。集約が2つ以上なら `| 集約 → 集約 | 手段 | 渡すもの | 一貫性 |` の表を空欄なしで置き、手段は 識別子で参照／値として渡す／ドメインイベント／呼び手が両方を操作 のどれか。集約が1つなら「なし」と書く。
     - `## BDDとの対応` は `| BDD | 要素 | 操作 | 成立を決める不変条件・事前条件 |` の表と、`- 対応しないBDD:` `- 対応のない要素・操作:` の2行を持つ。業務知識の全BDD番号が表か前者の行に、要素一覧の全要素が表か後者の行に現れる。業務知識に無いBDD番号を書かない。
     - `## 業務知識へ提案する概念` は `| 概念 | なぜ要るか | 導いた業務ルール・BDD | 業務知識のどの節へ足すか |` の表（空欄なし、同じ概念を2度書かない、引くBDD番号は業務知識にあるもの、足す先は業務知識の節名）か、「なし」だけ。提案した語は要素一覧の要素名と業務知識の語に現れない。
     - `## 未決` は空にしない（0件なら「なし」）。 -->
...
<!-- 書く: 元にした業務知識、扱うコアの範囲、対象外にした概念とその理由。
...
<!-- 書く: 全要素の名前、種別、業務知識のどの語に対応するか、一文の目的。
     書かない: 業務知識に無い語の要素、実装上の補助的な型。 -->
...
| 要素 | 種別 | 業務知識の語 | 目的（一文） |
|---|---|---|---|
| <要素名> | <種別> | <業務知識の語> | <何を成し遂げるためにあるか> |

## モデル図

<!-- 書く: 集約ごとに1枚、小見出しと小さなclassDiagramを置く（任意ではない）。小見出しの直下に、その集約の責務と境界で守る不変条件を書く。
     図には集約ルート・内側の要素・発するドメインイベントを描き、集約をまたぐ参照先は<<外部の集約>>として描く。要素一覧の全要素がどこかの集約の図に現れる。
     メソッドは公開コマンドだけ。集約が2つ以上なら「集約どうしの関係」の図を必ず1枚足し、集約が1つなら置かない。
     書かない: フィールド、ゲッター、判定だけの操作、型名、実装のクラス、複数の集約を1枚に混ぜた図。 -->

<!-- クラスのidはMermaidの識別子制約のためASCIIで書き、ラベル（["…"]）に業務知識の語をそのまま書く。idは図の中だけの名前で、実装の名前ではない。
...
<!-- 書く: ドメインイベント以外の全要素について、目的・何でないか・持つもの・不変条件・生成の条件・協働相手と、操作ごとの契約。
     書かない: 型名、クラス構文、getter/setter、永続化、シリアライズ形式。 -->

<!-- ドメインイベント以外の全要素に###を置く。6項目すべてを####で置き、該当が無い項目は「なし」と書く（空欄は不存在か見落としかを区別できない）。表に詰め込まない。
     操作は「#### 操作: <操作名>」を操作ごとに1つ以上置き、判断（事前条件）と変更（事後条件）を1つの操作として書く。
     事前条件を満たさないときの結果は「拒む理由」に、業務知識の「拒むときの理由」の語で書く。
...
<!-- ドメインイベントは、集約の操作の事後条件が成り立ったときに発する通知である。業務知識の業務イベントのうち、
...
<!-- 書く: 業務知識の各BDDが、どの要素のどの操作と不変条件で成立するか。対応しないBDDと、対応のない要素・操作。
...
<!-- 業務知識に無い語を要素にしたくなった案、形が同じ語を1つにまとめる案、
...
<!-- 書く: モデリング中に見つかった、業務知識に無い概念ごとに、名前、なぜ要るか、どの業務ルール・BDDから導いたか、業務知識のどの節へ足すべきか。
     この節の語は要素一覧に入れない。業務知識を直接書き換えず、業務知識の反証（formulate-domain）へ渡す提案として残す。0件なら「なし」と明記する。
     書かない: 要素にした語、採らなかった割り当て（捨てた割り当てへ）、実装の都合で欲しい型。 -->

<!-- 「足す先」は業務知識の節名（業務用語／業務イベント／概念／常に守られること／状態と、その移り変わり／誰が行えるか／業務ルール／拒むときの理由／まだ決まっていないこと／BDD）で書く。 -->

| 概念 | なぜ要るか | 導いた業務ルール・BDD | 業務知識のどの節へ足すか |
|---|---|---|---|
| <業務知識に無い語> | <この語が無いとモデルのどこが決まらないか> | <業務ルールの節名、BDD番号、業務知識の未決> | <業務知識の節名> |

## 未決

<!-- 書く: 業務知識の未決のうちモデルに影響するものと、モデル固有の未決。何が分かれば確定するか。
