...
> 型: クラウドアーキテクチャ ／ 読み手: 将来構成を決める設計者と運用責任者 ／ 更新日: 2026-09-14 ／ プロバイダー（機械値: `provider`）: AWS（機械値: `aws`）

**RoomFlowのプロバイダーはAWSで合意し、AWSの管理サービスを使った単一リージョン・複数AZ構成を第一候補とする。** 同じ利用枠への重複予約を防ぐデータベース整合性と、2人で運用できることを優先したためである。一方、年末の最大負荷とリージョン障害時の復旧目標が未決なので、AWS上の具体的な構成はTerraform実装へ渡せる最終決定ではない。

## 状態と引き継ぎ

構成候補と検証経路は説明できるが、`REQ-OQ-001`、`WL-OQ-001`、`QR-OQ-001`、`ARC-OQ-001`が残る。`status`は`unresolved`、`open_questions`はこの4件、`handoff.ready`は`false`、`blocked_by`も同じ4件とする。比較と検証準備は進めるが、`ADR-001`を承認済みとして実装しない。
