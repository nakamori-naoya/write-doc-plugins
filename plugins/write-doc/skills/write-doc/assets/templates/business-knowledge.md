...
<!-- 執筆指示: 仕組みを全部やめても残る業務の行いと決まりを、業務の言葉と、この資料で名前を一つに決めた語（ユビキタス言語）だけで書く。
     ...
     処理の遅れ、キャッシュ、画面、API、保存の仕方のような実装の関心は書かない。
...
<!-- 検査が読む目印: ほかの package の検査がこの型の資料から読むのは、ここに書いた目印だけである。見出しの文言は読まないので、見出しには節の結論を入れてよい。
     ユビキタス言語は、見出し行が `| 業務の言葉 | 英名 | 種類 | 持ち主 |` の表に1語1行で置く。種類は `業務用語`、`業務イベント`、`概念`、`状態`、`コマンド`、`クエリ` のどれか一つで、一つの語は一行だけに置く。業務用語でもある概念は `業務用語` として置く。
     持ち主は、その語を決めた資料がこの資料なら `この資料`、ほかの業務知識の資料が決めた語なら、その資料への相対 Markdown リンクにする。ほかの資料が決めた語は、持ち主の行と同じ言葉、英名、種類で置く。持ち主の資料がまだ無いときは、持ち主がリンクの行に限り、英名の欄に `未定` と書き、推測で英名を置かない。持ち主の資料ができたら、同じ入口で深めて外す。
     持ち主が作業の範囲の外にあり、資料が置かれないときは、持ち主の欄に `範囲の外: [<持ち主の資料の名前>](<パスかURL>)` と書く。その行はほかの資料と照合されず、持ち主の資料が英名を決めていなければ英名は `未定` でよい。
     英名は、語ごとに先頭だけを大文字にしてつなぐ。略語も一語として扱う（`OrderId` とし、`OrderID` にしない）。
     状態を持つものごとに、状態遷移図を1行目が `---` の Mermaid ブロックで描く。ブロックの先頭は `---`、`title: <状態を持つもの>`、`---` の三行で、その次の行が `stateDiagram-v2` である。遷移の行は `<状態> --> <状態>: <業務の行い>` にし、生成は `[*]` から始め、業務に終わりの状態があるときだけ `--> [*]` で閉じる。繰り返す対象は閉じない。状態の名前は、図に書いた状態名だけを使い、ユビキタス言語の表に種類 `状態` の行として置く。
     同時に起きたときの項目は `### 同時に起きること: <名前>` の見出しで一つずつ置き、その段落に、その項目を示す BDD の番号（`BDD-<番号>`）を書く。
...
<!-- 書く: 業務を変える行いは ## コマンド の下に、業務を変えずに知る行いは ## クエリ の下に、行いごとに ### を立てる。一つの行いの決まりは、この ### の下にだけ書く。
     ...
     表示対象と並び順を変えない値（金額の書式、一度にまとめて送る通知の数など）は書かず、「<値>は<置き場>に置いた」と一行だけ残す。置き場は API の契約か品質要求である。
     拒む理由は #### の見出しを一つずつ立て、その下の一段落に業務上の理由を書く。
     コマンドを持たず、隣の業務の事実を読むだけの業務なら、## コマンド の下に「なし」と一文で書く。
     ログインのように業務を変えずに誰かを知る行いも、クエリとして書く。表示対象は一人で、並び順は書かない。 -->
...
<!-- 書く: 同時に起きることごとに `### 同時に起きること: <名前>` を立て、業務上どちらが通るかと、その理由と、それを示す BDD の番号を一段落で書く。どの項目にも BDD を一件置き、勝った側の結果と、負けた側に何が起きないかを示す。
     読むだけの業務や状態の無い業務のように、ぶつかる判断が無い業務は、見出しの下に「なし」と一文で書けば足り、項目の見出しも BDD も置かない。 -->

### 同時に起きること: <名前>

<どちらが通るか。その理由。示す BDD の番号>

# まだ決まっていないこと

<!-- 書く: まだ決まっていない論点ごとに一項目を置く。答えを仮に置いた項目は、仮置きであること、その根拠、採らなかった案、決める人をそろえる。答えを置いていない項目は、何が分かれば決まるかと、決める人を書く。この形を満たせない項目は、決まっていないことを決まったことのように読ませるので書かない。無ければ「なし」と一文で書く。 -->
