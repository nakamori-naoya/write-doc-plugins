...
`current_version`は、その貸出の`loan_base_events`の最大の`version`と等しい（BDD-003、BDD-005）。
...
延滞にするのと本を返すのが同じ貸出で重なったら、先に記録したほうだけが成立する。後のほうは、読んだ`current_version`がもう変わっているので成立しない（BDD-005）。
...
### [BDD-003] 延滞の本を返すと返却済みになり、返却の事実が積まれる
...
### [BDD-004] 返却期限の翌日に延滞になり、通知の要求が同時に積まれる
