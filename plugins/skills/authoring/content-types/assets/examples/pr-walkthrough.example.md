...
2. **データベースへ占有を書き込む** — GiST排他制約が重なる利用枠を拒否する。
...
05   try {
06     // ［ロジック変更なし］仮押さえ予約と占有を同じトランザクションへ書く。
07     return await repository.createTentativeHold(input);
08   } catch (error) {
09     // ［新規］データベース制約が検出した競合だけを業務エラーへ変換する。
...
SQLSTATE `23P01`だけでは、将来追加される別の排他制約まで`SLOT_UNAVAILABLE`へ変換する可能性がある。PostgreSQLドライバーから制約名`room_booking_claims_room_time_excl`を失わず取得できるかは未確認である。
