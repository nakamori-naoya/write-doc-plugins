...
**この文書が、意味上の役から Markdown の記法への対応表の正本である。** 入力で付与済みの役と図だけを写し、役の選択・個数・位置・図の採否と型は判断しない。

## 強調

本文中の役の印（`⟦要点: …⟧` / `⟦キーワード: …⟧`）を次へ写す。
...
図の主張・図の型・図の内容は入力として受け取る。画像にするかも入力で確定済みとし、ここでは判断しない。
...
**画像として受け取った図は、`<資料名>.assets/<図の名前>.svg`（または `.png`）に置き、`![図の主張](<資料名>.assets/<図の名前>.svg)` と相対パスで参照する。** `<資料名>` は保存する Markdown の拡張子を除いた名前。`<図の名前>` は、入力で固定名が指定されていればその名前、それ以外は本文での登場順の `fig-NN` である。`<資料名>.assets` directoryを保存するMarkdownと同じdirectoryに置き、画像ファイルはそのassets directory内に置く。Markdownから画像へのリンクは相対pathにする。SVG 内の色は属性で直接指定する。
