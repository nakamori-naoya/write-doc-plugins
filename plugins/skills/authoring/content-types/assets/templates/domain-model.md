...
<!-- 執筆指示: 業務知識・コアドメインの正本に現れる業務概念を、値オブジェクト・エンティティ・集約・ドメインイベントへ割り当て、
     ...
     不変条件は3つ。
     1. 正本に無い語を要素にしない。要素の名前は正本のユビキタス言語にある語だけを使う。必要に思えたら「捨てた割り当て」か「未決」へ理由付きで書く。
...
> 型: ドメインモデル ／ 読み手: <業務を形にするエンジニア> ／ 入力: <業務知識・コアドメインの正本>
...
<!-- 書く: 集約ごとに1つの小見出しと1枚のclassDiagram。小見出しの直下に、その集約の責務（一文）と、境界で守る不変条件（何をこの内側で一貫させるか）。
     図には集約ルート・内側の要素・その集約が発するドメインイベントを描き、集約をまたぐ参照先は<<外部の集約>>のクラスとして描く。
     メソッドは公開コマンド（状態を変える操作と生成）だけ。集約が2つ以上なら、末尾に「集約どうしの関係」の図を1枚置く。
...
<!-- 要素ごとに### を1つ、項目ごとに#### を置く。表にしない。項目は省略せず、該当が無ければ「なし」と書く。
