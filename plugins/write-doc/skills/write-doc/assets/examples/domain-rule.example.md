...
| 仮押さえ予約を期限切れにする | コマンド | 仮押さえ期限が到来した |
| 予約待ちを繰り上げる | コマンド | 予約待ちから仮押さえ予約が成立した |
...
業務の信頼性を担保するため、いかなる場合でも破ってはならない不変条件を以下に定める。これらの不変条件は、例外処理や障害復旧の際にも常に優先され、データや操作の整合性を保つ境界線となる。
...
|---|---|---|---|
| 空いている利用枠を探す | クエリ | 利用登録済みの顧客 | 登録していない人は予約を申し込めないため、探す意味を持たない |
| 利用枠を仮押さえする | コマンド | 予約可能顧客である予約者 | 仮押さえ停止中顧客は新しい利用枠を確保できない |
| 仮押さえ予約を確定する | コマンド | その予約の予約者本人のみ | 第三者は本人の利用意思を示せない |
| 予約を取り消す | コマンド | その予約の予約者本人のみ | 第三者は本人の利用約束を終了できない |
| 予約待ちを登録する | コマンド | 予約可能顧客である予約者 | 仮押さえ停止中顧客は新しい利用希望を追加できない |
| 予約待ちを取り消す | コマンド | その予約待ちの予約者本人のみ | 第三者は本人の利用希望を終了できない |
| 仮押さえ予約を期限切れにする | コマンド | 予約業務（仮押さえ期限の到来で行う） | 期限前に期限切れにすると、予約者が確定する機会を失う |
| 予約待ちを繰り上げる | コマンド | 予約業務（確定予約の取消で行う） | 受付順以外で繰り上げると公平性を説明できない |
| 自分の予約と予約待ちを見る | クエリ | その予約・予約待ちの予約者本人のみ | 他人の利用予定は業務上知る必要がない |
| 予約待ちの順序を変更する | コマンド | 誰も行えない | 受付順を人が変えると公平性を説明できない |
