...
### roomflow-api: 利用者対向の同期処理とタスク起票
Cloud Run で稼働する Go 1.24 製のステートレスな API サービス。利用者のブラウザからの予約作成・確定・取消を受け付け、Cloud SQL と直接通信する。仮押さえ成立時には、自前でタイマーを持たず Cloud Tasks へ失効タスクを投げる責務を持つ。

### roomflow-worker: バックグラウンドの期限切れ確定処理
外部からのインバウンド通信を遮断した内部専用の Cloud Run サービス。Cloud Tasks から push されたタスクを契機に起動し、未確定の仮押さえを安全に「期限切れ」状態へ遷移させる。

### roomflow-db: 予約トランザクションと監査イベントの永続化
PostgreSQL 16（Cloud SQL）のインスタンス。予約枠の重複防止（行ロック）と、成立した業務イベントの記録を担う。プライベートIPのみで構成され、インターネットからの直接アクセスを完全に排除している。
