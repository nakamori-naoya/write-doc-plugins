...
| 本を借りている利用者を確かめる | FindBorrowerOfBook | クエリ | この資料 |
...
見せるのは、その利用者の貸出中と延滞の貸出のそれぞれについて、本の資料番号、返却期限、延滞かどうかである。返却済みの貸出は見せない。返却期限の近い順に並べ、同じ返却期限なら先に借りた本を先にする。期限の迫った本から返せるようにするためである。
...
図書館の窓口が行う。返ってきた本や、棚に無い本が誰の手元にあるかを確かめるためである。

見せるのは、その本の貸出中か延滞の貸出を持つ利用者一人の利用者番号である。一冊の本は同時に一つの貸出にしか属さないので、見えるのは一人か、誰もいないかのどちらかで、並び順は無い。
...
### 同時に起きること: 延滞にするのと本が返されるのが同じ貸出で重なる

先に起きた方が通る。返却が先なら延滞にしない。返却済みの貸出は変わらないからである（BDD-012）。

### 同時に起きること: 同じ本を二人の利用者が借りる

先に受け付けた一人だけが借りられる。一冊の本は一つの貸出にしか属さないからである（BDD-013）。

### 同時に起きること: 同じ利用者が5冊目と6冊目を同時に借りる

貸せるのは、先に受け付けた一冊だけである。貸出上限を超えて貸さないからである（BDD-014）。

# まだ決まっていないこと

貸出の延長を認めるかは決まっていない。認めるなら返却期限が変わるので、「返却期限は借りた日の14日後で、貸出の間変わらない」を見直す。決めるのは図書館の運営で、延長を求める利用者がどれだけいるかが分かれば決まる。
...
### [BDD-018] 延滞の本は、借りている利用者が見える

```gherkin
Given: 本 B-1003 は利用者 U-0001 の延滞の貸出である
When: 窓口が本 B-1003 を借りている利用者を確かめる
Then: 利用者 U-0001 が見える
```

### [BDD-019] 返却済みの本は、借りている利用者が見えない

```gherkin
Given: 本 B-1004 の貸出はどれも返却済みである
