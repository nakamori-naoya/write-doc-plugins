...
最後のreview工程で`review-doc`を呼び、保存済みのpath、読み手の前提、文書型、執筆時の確認記録、writing-rulesの解決済み設定を渡す。完成文書を基準に照らして評価する責務はreview-docが持つ。
...
- 通した工程の `id` を順に（`type` → `draft` → `visual` → `save` → `review`）
- 書いたファイルの**絶対パス**
